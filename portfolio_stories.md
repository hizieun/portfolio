...
| "다시 한다면 무엇을 바꾸겠나" | 현대캐피탈 (권한을 DB로 내렸을 것) | NeuroCore (LLM-as-Judge 도입) |
...
- **다시 한다면**: 현재 권한은 API 계층(Lambda)에서 evaluation. 다시 한다면 **OpenSearch 인덱스의 메타데이터 필드로 내려서 retrieval 단계에서 filter clause로 적용**했을 것. 불필요한 vector search 비용 ↓, 누락 위험 ↓
...
- "다시 한다면" → 권한을 retrieval index로 내렸을 것 (시니어 시그널)
