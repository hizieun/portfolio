...
내부 임직원용. 업무 매뉴얼·정책 문서가 주 데이터. 갱신 주기는 *상시* — 정책이 바뀌면 즉시 반영돼야 합니다. **고객상담 Agent**가 계좌 개설·수수료·서비스 절차 등 상담 지식 응답에 쓰는 코퍼스.

### 2) 펀드 / ELS 상품 임베딩 · *고객상담 Agent (금융상품 도메인)*
...
- **플랫폼 V2 전환** — 깨비AI가 차세대 아키텍처로 진화 중. 데이터 백본도 그 방향에 맞춰 함께 변경 예정 (상세는 진행 후 공유)
