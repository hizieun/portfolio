...
### 그래서 실제로 만들었습니다 — 사이드 프로젝트로 검증한 회고

이 회고를 말로만 남기지 않고, 사이드 프로젝트 [노포지도](https://github.com/hizieun/noodle-app/tree/main/eval)의 RAG 챗봇에 **평가 하네스를 직접 구현**했습니다. 골든셋 149문항(층화 124 + 적대적 엣지 25), 규칙 기반 환각 검출 + LLM-as-Judge 이중 판정, CI 회귀 게이트까지.

여기서 뉴로코어 때 놓쳤던 두 가지를 정면으로 보강했어요:

1. **표본 크기와 유의성** — "10문항의 +30%가 유의한가"에 답할 수 없던 구조를, *같은 세트를 k=3회 반복해 지표의 표준편차를 재고 회귀 임계값을 2σ로 데이터에 근거해 정하는* 구조로 바꿨습니다.
2. **측정 도구의 신뢰성** — 정확도가 올랐다는 숫자가 *계측기 고장이 아님*을 어떻게 보증하는가. **negative control**(합성 위반을 심어 탐지기가 실제로 켜지는지 검증)과 **고정 fixture**(프롬프트 포맷이 바뀌면 지표보다 추출기 테스트가 먼저 실패)로, 지표가 움직일 때 그것이 성능 변화임을 보장했습니다.

특히 *"환각률 0%"가 탐지기가 잘 만들어진 것인지 아무것도 안 하는 것인지 구분되지 않는다*는 함정 — 이건 뉴로코어 당시엔 인지조차 못 했던 문제입니다.
